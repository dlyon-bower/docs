...
- **CORS** — only approved origins can make requests to Bower's API. Wildcard origins are not allowed.
- **Security headers** — Content Security Policy, X-Content-Type-Options, and X-Frame-Options are set on all responses.
- **Signed file URLs** — files are never served from a public bucket. Every download uses a signed URL that expires after 24 hours.

## Secrets management

API keys, database credentials, and service account tokens are stored in a dedicated secrets manager — never in environment variables, source code, or configuration files. Application code accesses secrets through a centralised configuration layer that caches values in memory and never logs them.
