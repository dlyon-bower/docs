---
title: "Organising your workspace"
description: "Drag-and-drop, bulk actions, and the &#8942; menu — everything you need to keep your workspace tidy."
---

Bower gives you a few ways to move, copy, and tidy up your workspace. None of them require you to open a note to act on it.

## The page tree

The left-hand panel is your page tree — a hierarchical view of all your notes and collections. It's the main way to navigate and organise your workspace.

### Drag and drop

- **Move a note** — drag it from its current position to a new location in the tree. Drop it onto a collection to nest it inside.
- **Move a collection** — drag an entire collection (and everything inside it) to a new position.

Drag-and-drop works for both notes and collections.

## Multi-select bulk actions

For moving, copying, or deleting multiple items at once:

<Steps>
  <Step title="Select items">
    Click the checkboxes next to notes or collections to select them. In the table view, hold **Shift** and click to select all rows between two items.
  </Step>
  <Step title="Choose an action from the toolbar">
    A bulk action toolbar appears when items are selected:
    - **Move to** — move all selected items to a new parent
    - **Copy to** — copy all selected items to a collection (originals stay in place)
    - **Delete** — move all selected items to the archive
  </Step>
</Steps>

Bulk actions apply to everything in your selection. If you've selected a collection, all artifacts within it are included.

## The **&#8942;** menu

Every note and collection in the page tree has a **&#8942;** menu (the three-dot icon that appears on hover). This gives you per-item actions without needing to open the item first.

| Action | What it does |
| --- | --- |
| Rename | Edit the note or collection title inline |
| Move to | Move to a different collection |
| Copy to | Duplicate to a collection |
| Add to collection | Associate with a collection |
| Delete | Move to archive (with confirmation) |

## Undoing actions

When you move or copy items, a toast notification appears at the bottom of the screen with an **Undo** button. You have 5 seconds to undo the action before it's finalised.

## Deletion and the archive

When you delete a note or collection, it is **moved to the archive** — not permanently removed. Archived items remain available for **30 days**, after which they are automatically and permanently deleted.

- **Deleting a collection** archives all notes and sub-collections inside it.
- **Undo** — a toast notification appears after every delete with an **Undo** button. Click it to restore the item immediately.
- **Archive page** — go to **Settings > Security > Archive** to see all archived items, restore them individually, or permanently delete them early.
- **Restoring an orphan** — if the original parent collection is also archived, the restored item is placed at the workspace root.

After 30 days, archived items are permanently removed from the database and file storage. If you need to preserve data long-term, [export it](/organisation/export) before the 30-day window expires.
